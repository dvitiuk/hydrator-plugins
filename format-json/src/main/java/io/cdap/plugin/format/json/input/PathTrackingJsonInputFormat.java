...
public class PathTrackingJsonInputFormat extends PathTrackingInputFormat {
  ...
  private Schema getModifiedSchema(Schema schema, List<String> toNullableFields) {
    ...
    if (toNullableFields.isEmpty()) {
      return schema;
    }
    List<Schema.Field> fieldCopies = new ArrayList<>(schema.getFields().size());
    for (Schema.Field field : schema.getFields()) {
      if (toNullableFields.contains(field.getName()) && !field.getSchema().isNullable()) {
